...
st.caption("Singapore FSI Demo — Snowflake + Cortex AI")
...
with tab2:
    ...
    st.markdown("Review **flagged communications** from the surveillance system. Run AI-powered compliance assessment via **Cortex AI**.")
    st.divider()
    ...
    if flagged_df.empty:
        ...
        if labels:
            ...
            st.divider()
            if st.button("Review with Cortex AI", type="primary", use_container_width=True):
                with st.spinner("Cortex AI analyzing communication..."):
...
with tab4:
    ...
    st.markdown("Generate a **Suspicious Transaction Report (STR)** narrative using **Cortex AI** for any employee on the watchlist.")
    st.divider()
    ...
    if st.button("Generate STR with Cortex AI", type="primary", use_container_width=True):
        with st.spinner("Cortex AI generating Suspicious Transaction Report..."):
